...
export default ({ index, item, session, onRemoveItem, onVoteUp, onVoteDown }) => {
  const voteUp =
    item.voters && session.user && item.voters.filter(v => v.user.id === session.user.id).length === 0 ? (
  ...
  return (
    <tr>
      ...
      <td>{item.user && session.user && item.user.id != session.user.id && voteDown}</td>
      <td>
        {item.voters && item.voters.length != 0 ? (
          <span>{item.voters.map(vote => vote.value).reduce((total, v) => total + v)}</span>
        ) : (
          ''
        )}
      </td>
